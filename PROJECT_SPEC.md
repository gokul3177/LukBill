...
└── LukBill/                  # Root Application Workspace
...
- **User**: Clinic owners/doctors (name, email, passwordHash, clinicId, avatarUrl).
...
| Prefix | Purpose |
|--------|---------|
| `/api/auth` | Register, login, forgot password, verify OTP, reset password, update profile (`PUT /api/auth/profile`). |
| `/api/clinic` | Create or fetch clinic configuration. |
| `/api/patients` | Register patients, search patients (with regex sanitization), get patient history. |
...
| `/api/billing` | Generate final bills from prescriptions (with strict tenant isolation) and fetch billing history. |

---

## 6. Security & Business Logic
*   **Tenant Isolation:** All database queries across patients, inventory, prescriptions, and billing strictly validate `clinicId: req.clinicId` to ensure total data isolation between different clinics.
*   **Regex Input Sanitization:** Patient search queries sanitize special regex characters (`.*+?^${}()|[]\`) to prevent Regex Injection and ReDoS vulnerabilities.
*   **Doctor Profile & Personalization:** Supports custom avatar picture uploads (stored as Base64 in MongoDB) via an interactive modal in the top navigation, and displays a daily rotating motivational medical quote in the top-mid position.
*   **Mobile UX Optimization:** Features a sticky bottom navigation bar for touch devices (Home, Consult, Inventory, Profile) and responsive stacked card layouts for tables on screens < 640px.
...
3. **WebSocket Reconnection**: The frontend `HeyLuk.jsx` component lacks automatic exponential backoff/reconnection if the socket drops.
